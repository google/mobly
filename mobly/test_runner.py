...
class TestRunner(object):
  ...
  Attributes:
    self.results: The test result object used to record the results of this test
      run.
  """

  class _TestRunInfo(object):
    """Identifies one test class to run, which tests to run, and config to run it with."""
  ...
  def mobly_logger(self, alias='latest'):
    ...
    Args:
      alias: optional string, the name of the latest log alias directory to
        create. If a falsy value is specified, then the directory will not be
        created.
    ...
    logger.setup_test_logger(
        self._root_output_path, self._testbed_name, alias=alias)
    try:
  ...
  def add_test_class(self, config, test_class, tests=None, name_suffix=None):
    ...
    Args:
      ...
      tests: list of strings, optional list of test names within the class to
        execute.
