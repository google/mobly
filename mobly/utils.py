...
def find_files(paths, file_predicate):
  ...
  Args:
    ...
    file_predicate: A function that returns True if the file name and extension
      are desired.
  ...
  file_list = []
  for path in paths:
    for dir_path, _, files in os.walk(abs_path(path)):
      for file_name in files:
        name, ext = os.path.splitext(file_name)
        if file_predicate(name, ext):
          file_list.append((dir_path, name, ext))
  return file_list
...
def concurrent_exec(func, param_list, max_workers=30, raise_on_exception=False):
  ...
  Args:
    ...
    param_list: A list of iterables, each being a set of params to be passed
      into the function.
    max_workers: int, the number of workers to use for parallelizing the tasks.
      By default, this is 30 workers.
...
def start_standing_subprocess(cmd, shell=False, env=None):
  ...
  Args:
    ...
    env: dict, a custom environment to run the standing subprocess. If not
      specified, inherits the current environment. See subprocess.Popen() docs.
...
def find_subclasses_in_module(base_classes, module):
  ...
  Args:
    base_classes: list of classes, the base classes to look for the subclasses
      of in the module.
