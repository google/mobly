...
def compute_selected_tests(test_classes, selected_tests):
  ...
  values are lists of selected tests in those classes, e.g.
  .. code-block:: python
  ...
  None means all tests in that class are selected.

  Args:
    test_classes: list of strings, names of all the classes that are part of a
      suite.
    selected_tests: list of strings, list of tests to execute. If empty, all
      classes `test_classes` are selected.
