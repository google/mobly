...
class AttenuatorDevice:
  ...
  AttenuatorDevice for  RC-DAT attenuators.
  ...
  def open(self, host, port=23):
    ...
    self._telnet_client.cmd("*CLS", False)
    command = "RFCONFIG? ATTN 1"
    config_str = self._telnet_client.cmd(command)
    config_str = config_str.replace(command, "")
  ...
  def set_atten(self, idx, value):
    ...
    self._telnet_client.cmd("ATTN %s %s" % (idx + 1, value), False)
  ...
  def get_atten(self, idx=0):
    ...
    if idx + 1 > self.path_count or idx < 0:
      raise IndexError("Attenuator index out of range!", self.path_count, idx)
    telnet_cmd = "ATTN? " + str(idx + 1)
    atten_val_str = self._telnet_client.cmd(telnet_cmd)
    atten_val = float(atten_val_str.replace(telnet_cmd, ""))
    return atten_val
