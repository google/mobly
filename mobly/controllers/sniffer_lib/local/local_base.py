...
class SnifferLocalBase(sniffer.Sniffer):
    ...
    def start_capture(self,
        ...
        capture_dir = os.path.join(self._logger.log_path, "Sniffer-{}".format(
            self._interface))
        os.makedirs(capture_dir, exist_ok=True)
        self._capture_file_path = os.path.join(
            capture_dir, "capture_{}.pcap".format(
                logger.get_log_file_timestamp()))
    ...
    def stop_capture(self):
        ...
        utils.stop_standing_subprocess(
            self._process, kill_signal=signal.SIGINT)
        self._post_process()
