...
def get_instances_with_configs(configs):
  ...
  Args:
    configs: A list of dicts each representing the configuration of one Monsoon.
...
class MonsoonProxy(object):
  ...
  def CollectData(self):
    """Return some current samples.

    Call StartDataCollection() first.
    """
...
class MonsoonData(object):
  ...
  def __init__(self, data_points, timestamps, hz, voltage, offset=0):
    ...
    Args:
      ...
      offset: The number of initial data points to discard in calculations.
    """
  ...
  def save_to_text_file(monsoon_data, file_path):
    ...
    Args:
      monsoon_data: A list of MonsoonData objects to write to a text file.
      file_path: The full path of the file to save to, including the file name.
    """
    if not monsoon_data:
  ...
  def from_text_file(file_path):
    ...
    Args:
      file_path: The full path of the file load from, including the file name.
...
class Monsoon(object):
  ...
  def attach_device(self, dut):
    ...
    Args:
      dut: A controller object representing the device being powered by this
        Monsoon box.
    """
    self.dut = dut
  ...
  def set_voltage(self, volt, ramp=False):
    ...
    Args:
      ...
      ramp: If true, the output voltage will be increased gradually to prevent
  ...
  def take_samples(self, sample_hz, sample_num, sample_offset=0, live=False):
    ...
    try:
      return MonsoonData(
          current_values, timestamps, sample_hz, voltage, offset=sample_offset)
    except:
      return None
