...
def create(configs):
  ...
  Args:
    configs: A list of dicts, each representing a configuration for an Android
      device.
  ...
  for ad in ads:
    if ad.serial not in valid_ad_identifiers:
      raise DeviceError(
          ad, 'Android device is specified in config but is not attached.')
...
def list_fastboot_devices():
  """List all android devices connected to the computer that are in in fastboot mode.

  These are detected by fastboot.
...
def get_instances_with_configs(configs):
  ...
  Args:
    configs: A list of dicts each representing the configuration of one android
      device.
...
def filter_devices(ads, func):
  ...
  Args:
    ...
    func: A function that takes an AndroidDevice object and returns True if the
      device satisfies the filter condition.
...
def take_bug_reports(ads, test_name=None, begin_time=None, destination=None):
  ...
  Args:
    ads: A list of AndroidDevice instances.
    test_name: Name of the test method that triggered this bug report. If None,
      the default name "bugreport" will be used.
    begin_time: timestamp taken when the test started, can be either string or
      int. If None, the current time will be used.
    destination: string, path to the directory where the bugreport should be
      saved.
  """
...
class AndroidDevice(object):
  ...
  Attributes:
    ...
    log_path: A string that is the path where all logs collected on this android
      device should be stored.
    log: A logger adapted from root logger with an added prefix specific to an
      AndroidDevice instance. The default prefix is [AndroidDevice|<serial>].
      Use self.debug_tag = 'tag' to use a different tag in the prefix.
    adb_logcat_file_path: A string that's the full path to the adb logcat file
      collected, if any.
    adb: An AdbProxy object used for interacting with the device via adb.
    fastboot: A FastbootProxy object used for interacting with the device via
      fastboot.
  ...
  @property
  def debug_tag(self):
    """A string that represents a device object in debug info.

    Default value is the device serial.
  ...
  @property
  def build_info(self):
    """Get the build info of this Android device, including build id and build type.
  ...
  def generate_filename(self,
    ...
    Args:
      ...
      time_identifier: string or RuntimeTestInfo. If a `RuntimeTestInfo` is
        passed in, the `signature` of the test case will be used. If a string is
        passed in, the string itself will be used. Otherwise the current
  ...
  def take_bug_report(self,
    ...
    Args:
      ...
      begin_time: Timestamp of when the test started. If not set, then this will
        default to the current time.
      timeout: float, the number of seconds to wait for bugreport to complete,
        default is 5min.
      destination: string, path to the directory where the bugreport should be
        saved.
  ...
  def run_iperf_client(self, server_host, extra_args=''):
    ...
    Args:
      ...
      extra_args: A string representing extra arguments for iperf client, e.g.
        '-i 1 -t 30'.
