...
class AdbError(Error):
  ...
  Args:
    ...
    serial: string, the serial of the device the command is executed on. This is
      an empty string if the adb command is not specific to a device.
  """
...
class AdbProxy(object):
  ...
  def _exec_cmd(self, args, shell, timeout, stderr):
    ...
    Args:
      args: string or list of strings, program arguments. See subprocess.Popen()
        documentation.
  ...
  def _construct_adb_cmd(self, raw_name, args, shell):
    ...
    Args:
      raw_name: string, the raw unsanitized name of the adb command to format.
  ...
  def instrument(self, package, options=None, runner=None, handler=None):
    ...
    Args:
      ...
      handler: optional func, when specified the function is used to parse the
        instrumentation stdout line by line as the output is generated;
        otherwise, the stdout is simply returned once the instrumentation is
        finished.
