...
class EventDispatcher:
  ...
  def pop_event(self, event_name, timeout=DEFAULT_TIMEOUT):
    ...
    Args:
      ...
      timeout: Number of seconds to wait when event is not present. Never times
        out if None.
  ...
  def pop_events(self, regex_pattern, timeout):
    ...
    Args:
      regex_pattern: The regular expression pattern that an event name should
        match in order to be popped.
      timeout: Number of seconds to wait for events in case no event matching
        the condition exits when the function is called.
  ...
  def _match_and_pop(self, regex_pattern):
    """Pop one event from each of the event queues.

    The event whose names match (in a sense of regular expression) regex_pattern
    will be popped.
    """
    results = []
    self.lock.acquire()
  ...
  def handle_event(self,
    ...
    Args:
      event_handler: Handler for the event, which should take at least one
        argument - the event json object.
      event_name: Name of the event to be handled.
      user_args: User arguments for the handler; to be passed in after the event
        json.
      event_timeout: Number of seconds to wait for the event to come.
      cond: A condition to wait on before executing the handler. Should be a
        threading.Event object.
      cond_timeout: Number of seconds to wait before the condition times out.
