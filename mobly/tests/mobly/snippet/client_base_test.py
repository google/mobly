...
from mobly.tests.lib import jsonrpc_client_test_base
from mobly.tests.lib.snippet import mock_socket_file
from mobly.tests.lib.snippet import utils as snippet_utils
import unittest
...
class FakeClient(client_base.ClientBase):
  ...
  def __init__(self):
    ...
    super().__init__(snippet_name='FakeClient', device=mock_device)
...
class ClientBaseTest(unittest.TestCase):
  ...
  def test_rpc_precheck_fail(self, mock_parse_response, mock_send_request, mock_gen_request, mock_precheck):
    ...
    client = FakeClient()
    client.start_server()
    mock_precheck.side_effect = client_base.ServerDiedError(mock.Mock(), 'server_died')

    with self.assertRaises(client_base.ServerDiedError):
      client.some_rpc(1, 2)
  ...
  def test_parse_rpc_no_response(self):
    ...
    with self.assertRaisesRegex(client_base.ProtocolError, client_base.ProtocolError.NO_RESPONSE_FROM_SERVER):
  ...
  def test_parse_response_miss_fields(self):
    ...
    with self.assertRaisesRegex(client_base.ProtocolError, client_base.ProtocolError.RESPONSE_MISS_FIELD % 'id'):
      client._parse_rpc_response(10, 'some_rpc', MOCK_RESP_WITHOUT_ID)

    with self.assertRaisesRegex(client_base.ProtocolError, client_base.ProtocolError.RESPONSE_MISS_FIELD % 'result'):
      client._parse_rpc_response(10, 'some_rpc', MOCK_RESP_WITHOUT_RESULT)

    with self.assertRaisesRegex(client_base.ProtocolError, client_base.ProtocolError.RESPONSE_MISS_FIELD % 'error'):
      client._parse_rpc_response(10, 'some_rpc', MOCK_RESP_WITHOUT_ERROR)

    with self.assertRaisesRegex(client_base.ProtocolError, client_base.ProtocolError.RESPONSE_MISS_FIELD % 'callback'):
  ...
  def test_parse_response_id_mismatch(self):
    ...
    with self.assertRaisesRegex(client_base.ProtocolError, client_base.ProtocolError.MISMATCHED_API_ID):
