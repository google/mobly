  ...
  def dump(self, content, entry_type):
    ...
    with self._lock:
      ...
      with io.open(self._path, 'a', encoding='utf-8') as f:
        ...
        # output.
        yaml.safe_dump(
            new_content, f, explicit_start=True, allow_unicode=True, indent=4)
...
class ExceptionRecord(object):
  ...
  Attributes:
    ...
    extras: optional serializable, this corresponds to the `TestSignal.extras`
      field.
...
class TestResultRecord(object):
  ...
  def _test_end(self, result, e):
    ...
    Args:
      ...
      e: A test termination signal (usually an exception object). It can be any
        exception instance or of any subclass of mobly.signals.TestSignal.
    """
...
class TestResult(object):
  ...
  Attributes:
    requested: A list of strings, each is the name of a test requested by user.
  ...
  def add_controller_info_record(self, controller_info_record):
    ...
    Args:
      controller_info_record: ControllerInfoRecord object to be added to the
        result.
    """
