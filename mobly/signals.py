...
class TestSignal(Exception):
  ...
  This is used to signal the result of a test.

  Attributes:
    details: A string that describes the reason for raising this signal.
    extras: A json-serializable data type to convey extra information about a
      test result.
  """
  ...
  def __init__(self, details, extras=None):
    ...
    try:
      ...
      raise TestSignalError('Extras must be json serializable. %s is not.' %
                            extras)
