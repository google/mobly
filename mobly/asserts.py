...
def skip(reason, extras=None):
    ...
    Raises:
        signals.TestSkip is raised to mark a test method as skipped.
    """
    raise signals.TestSkip(reason, extras)


def skip_if(expr, reason, extras=None):
    """Skip a test method if expression evaluates to True.
...
def abort_class(reason, extras=None):
    """Abort all subsequent test methods within the same test class in one
    iteration.
...
def abort_all(reason, extras=None):
    """Abort all subsequent test methods, including the ones not in this test
    class or iteration.
...
def abort_all_if(expr, reason, extras=None):
    """Abort all subsequent test methods, if the expression evaluates to
    True.
...
def fail(msg, extras=None):
    ...
    Raises:
        signals.TestFailure is raised to mark a test method as failed.
    """
    raise signals.TestFailure(msg, extras)


def explicit_pass(msg, extras=None):
    """Explicitly pass a test method.
    ...
    Raises:
        signals.TestPass is raised to mark a test method as passed.
    """
