...
WINDOWS_RESERVED_CHARACTERS_REPLACEMENTS = {
    ...
    '?': ',',
    '*': ',',
    chr(0): '0',  # Integer zero (i.e. NUL) is not a valid character.
...
def create_latest_log_alias(actual_path, alias):
  ...
  Args:
    actual_path: string, the source directory where the latest test run's logs
      are.
...
def setup_test_logger(log_path, prefix=None, alias='latest'):
  ...
  Args:
    ...
    alias: optional string, The name of the alias to use for the latest log
      directory. If a falsy value is provided, then the alias directory will not
      be created, which is useful to save storage space when the storage system
      (e.g. ZIP files) does not properly support shortcut/symlinks.
  """
